...
| 필드 | 역할 |
|---|---|
| `look` | **픽셀 초상화 생성 파라미터** (`ui/portraits.ts`): 피부/눈/헤어스타일 7종/의상/칼라 5종/**체형 4종(slim·average·broad·heavy)**/**단신(petite)**/안경/속눈썹/립스틱/볼터치/귀걸이. 두 가지 샷을 생성 — **좌석용 웨이스트샷**(52×76, 체형·앉은 키 반영, 팔을 카운터에 올린 자세)과 **대화 카드용 바스트샷**(40×48 얼굴 클로즈업). 스쿱 드레스+slim은 모래시계 실루엣 |
...
**초기 로스터 (11명)**: 김과장(회사원) · 유리(대학생) · 마르코(여행객) · 서연(디자이너) · 박사장(옆가게 사장) · 하늘(소설가) · 민준(트레이너) · 지은(간호사) · 톰(영어강사) · 은자씨(어르신) · **세라(재즈 싱어)** — 취향이 15종 레시피와 티어를 고르게 커버하도록 배치.
...
**대화 인터랙션 카드**: greet/반응/화남 순간에는 하단 카운터 위로 **비주얼노벨식 카드**(픽셀 초상화 5배 확대 + 이름·직업 + 대사)가 3초간 표시된다. 좌석 스프라이트도 같은 초상화 텍스처(3배)를 사용해 일관성 유지. 초상화 전체 프리뷰는 `?portraits` 쿼리로 확인.

### 대화 상호작용 (호응/조언/침묵)

서빙 반응 후(점수 ≥0.35, 65% 확률) 손님이 말을 건다 — `talks` 데이터(페르소나당 3세트):

```
Talk { text: 손님의 이야기, options: [{ kind: sympathize|advise|silence, reply, affinity: -1|0|+1 }] }
```

- 카드에 🙌 호응 / 💡 조언 / 🤫 침묵 버튼이 뜨고, 선택하면 페르소나별 답변 + 호감도 증감(💗/💔 플로팅)
- **정답이 페르소나마다 다르다**: 지친 김과장에겐 호응이, 박사장에게 조언은 역효과("나한테 사람 관리를 가르치려고?"), 소설가 하늘은 침묵을 좋아함 — 손님을 알아가는 것이 곧 공략
- 8초 무응답 시 침묵으로 자동 처리, 대화 중에는 다른 대사가 카드를 덮지 않음(sticky)
- 상호작용 호감도는 서빙 호감도와 같은 풀에 누적 → 단골 레벨업으로 이어짐
